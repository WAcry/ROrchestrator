...
public sealed class ToolingJsonV1Tests
{
    ...
    public void DiffPatchJson_ShouldReportQosLimitsAndLimitKeyDiffs_AndProduceStableJson()
    {
        ...
        Assert.Equal(
            """{"kind":"diff","tooling_json_version":"v1","module_diffs":[{"kind":"limit_key_changed","flow_name":"HomeFeed","stage_name":"s1","module_id":"m1","path":"$.flows.HomeFeed.stages.s1.modules[0].limitKey","experiment_layer":null,"experiment_variant":null},{"kind":"limit_key_changed","flow_name":"HomeFeed","stage_name":"s1","module_id":"m_exp","path":"$.flows.HomeFeed.experiments[0].patch.stages.s1.modules[0].limitKey","experiment_layer":"l1","experiment_variant":"B"}],"qos_tier_diffs":[{"kind":"removed","flow_name":"HomeFeed","qos_tier":"conserve","path":"$.flows.HomeFeed.qos.tiers.conserve.patch"},{"kind":"changed","flow_name":"HomeFeed","qos_tier":"emergency","path":"$.flows.HomeFeed.qos.tiers.emergency.patch.stages.s1.fanoutMax"},{"kind":"changed","flow_name":"HomeFeed","qos_tier":"emergency","path":"$.flows.HomeFeed.qos.tiers.emergency.patch.stages.s1.modules[0].enabled"},{"kind":"changed","flow_name":"HomeFeed","qos_tier":"emergency","path":"$.flows.HomeFeed.qos.tiers.emergency.patch.stages.s1.modules[1].shadow.sample"},{"kind":"added","flow_name":"HomeFeed","qos_tier":"fallback","path":"$.flows.HomeFeed.qos.tiers.fallback.patch"}],"limit_diffs":[{"kind":"changed","limit_key":"depA","path":"$.limits.moduleConcurrency.maxInFlight.depA","old_max_in_flight":10,"new_max_in_flight":5},{"kind":"added","limit_key":"depB","path":"$.limits.moduleConcurrency.maxInFlight.depB","old_max_in_flight":null,"new_max_in_flight":1}],"param_diffs":[],"fanout_max_diffs":[],"emergency_diffs":[],"risk_report":{"level":"high","fanout_increase_count":0,"module_added_count":0,"module_removed_count":0,"shadow_change_count":0,"limit_key_change_count":2,"limit_change_count":2,"qos_change_count":5,"param_change_count":0,"emergency_change_count":0}}""",
            result.Json);
    }
                  ...
                  "emergency": {
                    ...
                    "patch": {
                      "stages": {
                        "s1": {
                          "fanoutMax": 1,
                          "modules": [ { "id": "m_disabled", "enabled": false } ]
                        }
                      }
                    }
                  }
                      ...
                      "modules": [
                        {
                          ...
                          "use": "test.module",
                          "with": {},
                          "gate": { "any": [ { "selector": "always_false" }, { "selector": "always_true" } ] }
                        },
                        {
                          "id": "m_comp_false",
                          "use": "test.module",
                          "with": {},
                          "gate": { "all": [ { "selector": "always_true" }, { "selector": "always_false" } ] }
                        }
                      ]
                    }
                  }
                  ...
                  "experiments": [
                    {
                      "layer": "l1",
                      "variant": "A",
                      "patch": { "params": { "a": 10, "nested": { "y": 20 }, "exp": true } }
                    }
                  ],
                  "qos": {
                    "tiers": {
                      "emergency": { "patch": { "params": { "a": 30, "nested": 5, "qos": true } } }
                    }
                  },
                  "emergency": {
                    "reason": "r",
                    "operator": "op",
                    "ttl_minutes": 30,
                    "patch": { "stages": {}, "params": { "a": 40, "em": "on" } }
                  }
                }
              }
            }
        ...
        var patchJson = """
            {
              ...
              "flows": {
                "HomeFeed": {
                  "params": { "b": 2, "a": 1 },
                  "experiments": [
                    { "layer": "l1", "variant": "A", "patch": { "params": { "a": 10, "c": true } } }
                  ]
                }
              }
            }

...
public sealed class PatchEvaluatorV1Tests
{
    ...
    public void Evaluate_WhenQosTierIsSelected_ShouldApplyQosPatch_BeforeEmergency()
    {
        ...
        using var evaluation = PatchEvaluatorV1.Evaluate("HomeFeed", patchJson, requestOptions, qosTier: QosTier.Emergency);

        Assert.Equal(4, evaluation.OverlaysApplied.Count);
        Assert.Equal("base", evaluation.OverlaysApplied[0].Layer);
        Assert.Equal("experiment", evaluation.OverlaysApplied[1].Layer);
        Assert.Equal("qos", evaluation.OverlaysApplied[2].Layer);
        Assert.Equal("emergency", evaluation.OverlaysApplied[3].Layer);
        ...
        Assert.Equal(2, stage.Modules.Count);
        Assert.Equal("m1", stage.Modules[0].ModuleId);
        Assert.False(stage.Modules[0].Enabled);
        Assert.True(stage.Modules[0].DisabledByEmergency);

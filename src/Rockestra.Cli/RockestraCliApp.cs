...
public static class RockestraCliApp
{
    ...
    private static void WriteCommandHelp(Utf8JsonWriter writer, string command)
    {
        ...
        writer.WriteString(
            ...
            {
                "validate" =>
                    "rock validate --bootstrapper <path> --bootstrapper-type <type> (--patch <path> | --patch-json <json>)",
                "explain-flow" =>
                ...
                "preview-matrix" =>
                    "rock preview-matrix --flow <name> (--patch <path> | --patch-json <json>) (--matrix <path> | --matrix-json <json>) [--user-id <value>] [--request-attr <k=v> ...] [--qos-tier <tier>] [--include-mermaid] [--bootstrapper <path>] [--bootstrapper-type <type>]",
                _ => "rock <command> [options]",
            });
